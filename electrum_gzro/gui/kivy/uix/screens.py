...
from electrum_gzro.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds, Fiat
from electrum_gzro.util import PR_TYPE_ONCHAIN, PR_TYPE_LN
from electrum_gzro import bitcoin, constants
from electrum_gzro.transaction import TxOutput, Transaction, tx_from_str
from electrum_gzro.util import send_exception_to_crash_reporter, parse_URI, InvalidBitcoinURI
from electrum_gzro.util import PR_UNPAID, PR_PAID, PR_UNKNOWN, PR_EXPIRED, TxMinedInfo, get_request_status, pr_expiration_values
from electrum_gzro.plugin import run_hook
from electrum_gzro.wallet import InternalAddressCorruption
from electrum_gzro import simple_config
from electrum_gzro.lnaddr import lndecode
from electrum_gzro.lnutil import RECEIVED, SENT, PaymentFailure

from .dialogs.question import Question
from .dialogs.lightning_open_channel import LightningOpenChannelDialog

from electrum_gzro.gui.kivy.i18n import _

if TYPE_CHECKING:
    from electrum_gzro.gui.kivy.main_window import ElectrumWindow
...
class CScreen(Factory.Screen):
    ...
    def load_screen(self):
        self.screen = Builder.load_file('electrum_gzro/gui/kivy/uix/ui_screens/' + self.kvname + '.kv')
...
class HistoryScreen(CScreen):
    ...
    def get_card(self, tx_item): #tx_hash, tx_mined_status, value, balance):
        ...
        if is_lightning:
            ...
            icon = "atlas://electrum_gzro/gui/kivy/theming/light/" + TX_ICONS[status]
...
class SendScreen(CScreen):
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Gravity address or a payment request'))
            return
        ...
        if self.screen.is_lightning:
            ...
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid Gravity Address') + ':\n' + address)
                return
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_gzro.util import create_bip21_uri
